...
def download_model():
    ...
    if MODEL_PATH.exists():
        ...
        if file_size < MIN_EXPECTED_SIZE:
            print(f"⚠️ Model found at {MODEL_PATH}, but it's too small ({file_size} bytes). Redownloading...")
            MODEL_PATH.unlink()
        else:
    ...
    try:
        response = requests.get(MODEL_URL, headers=headers, stream=True, allow_redirects=True, timeout=30)
    ...
    if total < MIN_EXPECTED_SIZE:
        print(f"❌ Content length too small ({total} bytes). This may indicate a bad URL or an authentication issue.")
        sys.exit(1)
    ...
    if final_size < MIN_EXPECTED_SIZE:
        print(f"❌ Final model size too small ({final_size} bytes). Deleting corrupt file.")
